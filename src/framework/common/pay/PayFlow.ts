	...
	export class PayFlowMG implements IPayFlow {
		...
		initConfig(parent: Parent) {
			...
			if (gdk.pluginName == "app" || gdk.pluginName == "appv2") {
				...
				if (gdk.nativeVersion <= 0) {
					this._appPayFlowMap["NativeVersionLessThan0"] = new PayInsideLocalV1.PayFlow()
				} else {
					if (gdk.nativeVersion >= 2) {
						this._appPayFlowMap["WechatPay"] = new PayInApp.PayFlow()
					} else {
						this._appPayFlowMap["WechatPay"] = new PayOutsideWithOrder.PayFlow()
					}
					if (gdk.nativeVersion >= 3) {
						this._appPayFlowMap["AliPay"] = new PayInAppWithAutoMakeup.PayFlow()
					} else {
						this._appPayFlowMap["AliPay"] = new PayOutsideWithOrder.PayFlow()
					}

...
declare namespace swan {
	...
	export interface BaiduOrderParams {
		dealId: string//		商户id。	
		appKey: string//		百度电商开放平台 appKey，用以表示应用身份的唯一 ID	
		totalAmount: string//		订单金额
		tpOrderId: string//		商户平台自己记录的订单 ID
		dealTitle: string//		订单的名称。	
		signFieldsRange: number//		固定值 1。	
		rsaSign: string//	对 appKey，dealId，tpOrderId，totalAmount RSA 加密后的签名
		bizInfo?: string //订单详细信息，需要是一个可解析为 JSON Object 的字符串
	}
	/**
	 * [发起baidu支付](https://smartprogram.baidu.com/docs/game/api/openApi/requestPolymerPayment/)
	 */
	export function requestPolymerPayment(req: {
		orderInfo: BaiduOrderParams
	...
	export class CustomerServiceConversationParams {
		/**
		 * 	''	否	会话来源
		 */
		sessionFrom?: string
		/**
		 * 	false	否	是否显示会话内消息卡片，设置此参数为 true，用户进入客服会话之后会收到一个消息卡片，通过以下三个参数设置卡片的内容
		 */
	...
	export function openCustomerServiceConversation(object: CustomerServiceConversationParams)
	...
	export interface AppCallUpParams extends BaseOptions {
		/**
		 * 要打开的小程序 appId
		 **/
		appId: string

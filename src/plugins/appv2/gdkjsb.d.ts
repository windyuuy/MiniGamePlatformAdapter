declare namespace gdkjsb {
	...
	export class Bridge {
		 ...
		 * @param action 动作
		 */
		registerAction(name: string, action: (/** 收到的数据 */ data: string, /**回掉的函数指针 */ callbackIndex: number) => void);
		 ...
		 * @param data 返回结果数据
		 */
		actionCallback(callbackIndex: number, data: string);
		 ...
		 * @returns action是否存在
		 */
		callAction(action: string, data: string, callback: (/**返回结果 */data: string, func: string) => void): boolean;
		 ...
		 * @returns 事件id（用于取消）
		 */
		on(eventName: string, callback: (data: string) => void): number;
		 ...
		 * @param action 动作名称
		 */
		checkActionExist(action: string): boolean;
	}

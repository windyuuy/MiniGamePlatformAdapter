namespace AppGDK {
	...
	export class Advert implements GDK.IAdvert {
		...
		async initWithConfig?(_info: GDK.GDKConfig) {
			...
			info.webview.advertPlatforms = info.webview.advertPlatforms || []
			if (info.webview.advertPlatforms.length == 0) {
				info.webview.advertPlatform = info.webview.advertPlatform || 'ironsource'
			}
			if (info.webview.advertPlatform) {
				info.webview.advertPlatforms.remove(info.webview.advertPlatform)
				info.webview.advertPlatforms.push(info.webview.advertPlatform)
			}
			for (let key of info.webview.advertPlatforms) {
				// 选择广告平台
			...
			// // 选择广告平台
			// if (info.webview && info.webview.advertPlatform != undefined) {
			// 	await SDKProxy.nativeAdvert.advertPlatformSelect(info.webview.advertPlatform)
			// } else {

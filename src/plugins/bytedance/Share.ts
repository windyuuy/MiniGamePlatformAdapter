namespace BytedanceGDK {
	...
	export class Share extends GDK.ShareBase {
				...
				let onShow = () => {
					...
					let shareMaySuc = () => {
						 ...
						 *  一定概率（配表）判定本次分享成功（每天第一次必然成功）
							一定概率（配表）判定本次分享失败，返回提示 调用头条分享接口失败,请重试，采用头条的提示弹框
							一定概率（配表）判定本次分享失败，返回提示 请不要频繁打扰同个用户/群哦，采用头条的提示弹框
						* */
		...
		/**
		 * 第二代头条分享解决方案
		 * * 通过头条分享按钮的cancel，来判定是否收到了分享取消回调。取消则必然失败
		 * * 提供一定的概率来判断分享
		 * 
		...
		/**
		 * 第三代头条分享解决方案
		 * * 通过头条的活动卡片，服务器修改活动卡片信息，如果修改成功则说明分享成功
		 * @param data 
		 */

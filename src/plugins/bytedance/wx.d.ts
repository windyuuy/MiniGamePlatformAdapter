...
declare namespace wx {
	...
	/**
	 * 发起网络请求。`wx.request`发起的是https请求。**一个头条小程序，同时只能有5个网络请求连接**。
	 */
	 ...
	 * **一个头条小程序同时只能有一个 WebSocket 连接，如果当前已存在一个 WebSocket 连接，会自动关闭该连接，并重新创建一个 WebSocket 连接**。
	 */
	...
	export interface RequestPaymentOptions extends BaseOptions {
		...
		/**
		 * 签名,具体签名方案参见[头条公众号支付帮助文档](https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=4_3&t=1477656495417)
		 */
		paySign: string;
	}
	...
	export class AuthSetting {
		...
		/**
		 * 是否授权头条运动步数，对应接口 wx.getWeRunData
		 */
		"scope.werun": boolean
